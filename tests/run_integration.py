"""Run real database tests against a disposable local MariaDB, never server.cfg."""
import os
...
instance = Path(tempfile.mkdtemp(prefix='feather-mysql-integration-'))
...
try:
    ...
    result = subprocess.run(['node', 'tests/integration.js'], cwd=root, env=env)
    raise SystemExit(result.returncode)
finally:
    # Also stop a replacement instance created by the restart test, if present.
    pid_file = instance / 'mysql.pid'
    if pid_file.exists():
        try:
    ...
    if process is not None:
        try:
            ...
            process.kill()
            process.wait()
    print('Private MariaDB stopped; test files/logs retained at', instance, flush=True)
